...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  // Handle relay control message
  if (strcmp(topic, "relayControl") == 0) {
    if ((char)payload[0] == '1') {
      Serial.println("Relay turned ON by MQTT");
      digitalWrite(relay, HIGH);
      delay(2500);
    } else if ((char)payload[0] == '0') {
      Serial.println("Relay turned OFF by MQTT");
...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    if (client.connect(clientId.c_str())) {
      ...
      client.subscribe("relayControl");  // Subscribe to relay control topic

      // Display heart pattern on successful connection
...
// Main loop function
void loop() {
  float tempC = ss.getTemp(); // Reads the temperature
  uint16_t capread = ss.touchRead(0); // Reads the moisture
  int roundedTemp = round(tempC);  // Round to the nearest integer
  ...
  if (now - lastMsg > 30000) {
    ...
    client.publish("/home/sensors", output); // Publish sensor data to MQTT
    Serial.println("Sent");
  }

  // Control relay based on moisture level and button state
